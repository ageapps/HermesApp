...
public class RCSignals {
    ...
    public void addThrottle(int delta) {
       // if (rc_signals_raw[THROTTLE] < 2000 - ThrottleLimit && rc_signals_raw[THROTTLE] > 0) {
            double increase = -delta/10;

            rc_signals_raw[THROTTLE] = rc_signals_raw[THROTTLE] + (int)increase;
            Log.i("RCSignals", "Increase: " + increase);
            Log.i("RCSignals", "Throttle: " + rc_signals_raw[THROTTLE]);
       // }
